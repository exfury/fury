...
LOGLEVEL="info"
# Set dedicated home directory for the furyd instance
HOMEDIR="$HOME/.tmp-furyd"
# to trace evm
#TRACE="--trace"
TRACE=""
...
while [[ $# -gt 0 ]]; do
	...
	-n)
		...
		;;
	--no-install)
		echo "Flag --no-install passed -> Skipping installation of the furyd binary."
		install=false
		shift # Move past the flag
		;;
	*)
...
if [[ $overwrite == "y" || $overwrite == "Y" ]]; then
	...
	# Set client config
	furyd config keyring-backend "$KEYRING" --home "$HOMEDIR"
	furyd config chain-id "$CHAINID" --home "$HOMEDIR"

	# myKey address 0x7cb61d4117ae31a12e393a1cfa3bac666481d02e | fury10jmp6sgh4cc6zt3e8gw05wavvejgr5pwjnpcky
	VAL_KEY="mykey"
	VAL_MNEMONIC="gesture inject test cycle original hollow east ridge hen combine junk child bacon zero hope comfort vacuum milk pitch cage oppose unhappy lunar seat"

	# dev0 address 0xc6fe5d33615a1c52c08018c47e8bc53646a0e101 | fury1cml96vmptgw99syqrrz8az79xer2pcgp84pdun
	USER1_KEY="dev0"
	USER1_MNEMONIC="copper push brief egg scan entry inform record adjust fossil boss egg comic alien upon aspect dry avoid interest fury window hint race symptom"

	# dev1 address 0x963ebdf2e1f8db8707d05fc75bfeffba1b5bac17 | fury1jcltmuhplrdcwp7stlr4hlhlhgd4htqh3a79sq
	USER2_KEY="dev1"
	USER2_MNEMONIC="maximum display century economy unlock van census kite error heart snow filter midnight usage egg venture cash kick motor survey drastic edge muffin visual"

	# dev2 address 0x40a0cb1C63e026A81B55EE1308586E21eec1eFa9 | fury1gzsvk8rruqn2sx64acfsskrwy8hvrmafqkaze8
	USER3_KEY="dev2"
	...
	# dev3 address 0x498B5AeC5D439b733dC2F58AB489783A23FB26dA | fury1fx944mzagwdhx0wz7k9tfztc8g3lkfk6rrgv6l
	USER4_KEY="dev3"
	USER4_MNEMONIC="doll midnight silk carpet brush boring pluck office gown inquiry duck chief aim exit gain never tennis crime fragile ship cloud surface exotic patch"

	# Import keys from mnemonics
	echo "$VAL_MNEMONIC" | furyd keys add "$VAL_KEY" --recover --keyring-backend "$KEYRING" --algo "$KEYALGO" --home "$HOMEDIR"

	# Store the validator address in a variable to use it later
	node_address=$(furyd keys show -a "$VAL_KEY" --keyring-backend "$KEYRING" --home "$HOMEDIR")

	echo "$USER1_MNEMONIC" | furyd keys add "$USER1_KEY" --recover --keyring-backend "$KEYRING" --algo "$KEYALGO" --home "$HOMEDIR"
	echo "$USER2_MNEMONIC" | furyd keys add "$USER2_KEY" --recover --keyring-backend "$KEYRING" --algo "$KEYALGO" --home "$HOMEDIR"
	echo "$USER3_MNEMONIC" | furyd keys add "$USER3_KEY" --recover --keyring-backend "$KEYRING" --algo "$KEYALGO" --home "$HOMEDIR"
	echo "$USER4_MNEMONIC" | furyd keys add "$USER4_KEY" --recover --keyring-backend "$KEYRING" --algo "$KEYALGO" --home "$HOMEDIR"

	# Set moniker and chain-id for Fury (Moniker can be anything, chain-id must be an integer)
	furyd init $MONIKER -o --chain-id "$CHAINID" --home "$HOMEDIR"
	...
	furyd add-genesis-account "$(furyd keys show "$VAL_KEY" -a --keyring-backend "$KEYRING" --home "$HOMEDIR")" 100000000000000000000000000afury --keyring-backend "$KEYRING" --home "$HOMEDIR"
	furyd add-genesis-account "$(furyd keys show "$USER1_KEY" -a --keyring-backend "$KEYRING" --home "$HOMEDIR")" 1000000000000000000000afury --keyring-backend "$KEYRING" --home "$HOMEDIR"
	furyd add-genesis-account "$(furyd keys show "$USER2_KEY" -a --keyring-backend "$KEYRING" --home "$HOMEDIR")" 1000000000000000000000afury --keyring-backend "$KEYRING" --home "$HOMEDIR"
	furyd add-genesis-account "$(furyd keys show "$USER3_KEY" -a --keyring-backend "$KEYRING" --home "$HOMEDIR")" 1000000000000000000000afury --keyring-backend "$KEYRING" --home "$HOMEDIR"
	furyd add-genesis-account "$(furyd keys show "$USER4_KEY" -a --keyring-backend "$KEYRING" --home "$HOMEDIR")" 1000000000000000000000afury --keyring-backend "$KEYRING" --home "$HOMEDIR"
	...
	furyd gentx "$VAL_KEY" 1000000000000000000000afury --keyring-backend "$KEYRING" --chain-id "$CHAINID" --home "$HOMEDIR"
	...
	# Collect genesis tx
	furyd collect-gentxs --home "$HOMEDIR"

	# Run this to ensure everything worked and that the genesis file is setup correctly
	furyd validate-genesis --home "$HOMEDIR"
