...
import (
	...
	fury "github.com/exfury/fury/v15/types"
...
func LegacyEIP712Cmd() *cobra.Command {
	return &cobra.Command{
		...
		Example: fmt.Sprintf(`$ %s debug legacy-eip712 tx.json --chain-id highbury_710-1`, version.AppName),
		Args:    cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			chainID, err := fury.ParseChainID(clientCtx.ChainID)
			if err != nil {
