...
import (
	...
	"github.com/exfury/fury/v15/testutil/network"
)
...
Example:
	furyd testnet init-files --v 4 --output-dir ./.testnets --starting-ip-address 192.168.10.2
	`,
...
Example:
	furyd testnet --v 4 --output-dir ./.testnets
	`,
		RunE: func(cmd *cobra.Command, _ []string) error {
			args := startArgs{}
			args.outputDir, _ = cmd.Flags().GetString(flagOutputDir)
...
func initTestnetFiles(
	...
	for i := 0; i < args.numValidators; i++ {
		...
		accStakingTokens := sdk.TokensFromConsensusPower(5000, furytypes.PowerReduction)
		coins := sdk.Coins{
			sdk.NewCoin(furytypes.AttoFury, accStakingTokens),
		}
		...
		valTokens := sdk.TokensFromConsensusPower(100, furytypes.PowerReduction)
		createValMsg, err := stakingtypes.NewMsgCreateValidator(
			sdk.ValAddress(addr),
			valPubKeys[i],
			sdk.NewCoin(furytypes.AttoFury, valTokens),
		...
		customAppTemplate, customAppConfig := config.AppConfig(furytypes.AttoFury)
	...
	if err := initGenFiles(clientCtx, mbm, args.chainID, furytypes.AttoFury, genAccounts, genBalances, genFiles, args.numValidators); err != nil {
		return err
	}
