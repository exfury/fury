...
def update_node_cmd(path, cmd, i):
    ...
    for section in ini.sections():
        if section == f"program:fury_9000-1-node{i}":
            ini[section].update(
                {
...
def custom_fury(tmp_path_factory):
    ...
    cmd = [
        "nix-build",
        "--no-out-link",
        Path(__file__).parent / "configs/broken-furyd.nix",
    ]
    print(*cmd)
    broken_binary = Path(subprocess.check_output(cmd).strip().decode()) / "bin/furyd"
    print(broken_binary)
...
def test_rollback(custom_fury):
    ...
    update_node_cmd(custom_fury.base_dir, "furyd", 1)
