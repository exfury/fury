...
def test_ibc_transfer_with_hermes(ibc):
    ...
    dst_addr = ibc.chains["fury"].cosmos_cli().address("signer2")
    old_dst_balance = get_balance(ibc.chains["fury"], dst_addr, dst_denom)
    new_dst_balance = 0

    def check_balance_change():
        nonlocal new_dst_balance
        new_dst_balance = get_balance(ibc.chains["fury"], dst_addr, dst_denom)
    ...
    fee = int(events["tx"]["fee"].removesuffix("afury"))
...
def test_fury_ibc_transfer(ibc):
    """
    test sending afury from fury to crypto-org-chain using cli.
    """
    assert_ready(ibc)
...
def test_fury_ibc_transfer_acknowledgement_error(ibc):
    """
    test sending afury from fury to crypto-org-chain using cli
