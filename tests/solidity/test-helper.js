...
function checkTestEnv () {
  const argv = yargs(hideBin(process.argv))
    ...
    .example(
      '$0 --network fury --allowTests=test1,test2',
      'run only test1 and test2 using fury network'
    )
    .help('h')
    .alias('h', 'help')
    ...
    .boolean('verbose-log')
    .describe('verbose-log', 'print furyd output, default false').argv
...
function setupNetwork ({ runConfig, timeout }) {
  ...
  const spawnPromise = new Promise((resolve, reject) => {
    ...
    const serverStartedMsg = 'furyd started'

    const furydProc = spawn('../e2e/init-node.sh', {
      cwd: __dirname,
      stdio: ['ignore', 'pipe', 'pipe']
    })

    logger.info(`Starting furyd process... timeout: ${timeout}ms`)
    if (runConfig.verboseLog) {
      furydProc.stdout.pipe(process.stdout)
    }

    furydProc.stdout.on('data', (d) => {
  ...
  const timeoutPromise = new Promise((resolve, reject) => {
    setTimeout(() => reject(new Error('Start furyd timeout!')), timeout)
  })
