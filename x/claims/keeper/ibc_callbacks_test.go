...
func (suite *KeeperTestSuite) TestAckknowledgementPacket() {
	...
	testCases := []struct {
		...
		},
		{
			...
			func() {
				addr, err := sdk.AccAddressFromBech32("fury1x2w87cvt5mqjncav4lxy8yfreynn273xn5335v")
				suite.Require().NoError(err)

				mockpacket.Data = transfertypes.ModuleCdc.MustMarshalJSON(
					&transfertypes.FungibleTokenPacketData{
						Sender:   "fury1x2w87cvt5mqjncav4lxy8yfreynn273xn5335v",
...
func (suite *KeeperTestSuite) TestOnRecvPacket() {
	...
	senderStr := "fury1sv9m0g7ycejwr3s369km58h5qe7xj77hvcxrms"
	receiverStr := "fury1hf0468jjpe6m6vx38s97z2qqe8ldu0njdyf625"
	...
	testCases := []struct {
		...
		},
		{
			"fail - invalid sender",
			func() {
				transfer := transfertypes.NewFungibleTokenPacketData("afury", "100", "fury", receiverStr, "")
				bz := transfertypes.ModuleCdc.MustMarshalJSON(&transfer)
				packet := channeltypes.NewPacket(bz, 1, transfertypes.PortID, "channel-0", transfertypes.PortID, "channel-0", timeoutHeight, 0)
		...
		},
		{
			...
			func() {
				transfer := transfertypes.NewFungibleTokenPacketData("afury", "100", secpAddrCosmos, secpAddrFury, "")
				bz := transfertypes.ModuleCdc.MustMarshalJSON(&transfer)
				packet := channeltypes.NewPacket(bz, 1, transfertypes.PortID, "channel-0", transfertypes.PortID, "channel-0", timeoutHeight, 0)
		...
		},
		{
			...
			func() {
				transfer := transfertypes.NewFungibleTokenPacketData("afury", types.IBCTriggerAmt, senderStr, receiverStr, "")
				bz := transfertypes.ModuleCdc.MustMarshalJSON(&transfer)
				packet := channeltypes.NewPacket(bz, 1, transfertypes.PortID, "channel-0", transfertypes.PortID, "channel-0", timeoutHeight, 0)
